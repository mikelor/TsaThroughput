    ...
    class ProgramTabula
    {
        ...
        public static async Task<int> Main(string[] args)
        {
            ...
            rootCommand.Handler = CommandHandler.Create<string, string>(async (inputFile, outputFile) =>
            {
                ...
                TsaThroughput tsaThroughput = new()
                {
                ...
                // Use Tabula
                using (PdfDocument document = PdfDocument.Open(tsaThroughputInputFile, new ParsingOptions() { ClipPaths = false }))
                {
        ...
        private static Airport AddAirport(TsaThroughput tsaThroughput, IReadOnlyList<Cell> row, string currentDateString, string currentHourString, ref int currentCell)
        {
        ...
        private static Checkpoint AddCheckpoint(TsaThroughput tsaThroughput, Airport airport, IReadOnlyList<Cell> row, string currentDateString, string currentHourString, ref int currentCell)
        {
            // Instantiate a checkpoint object from the current cell
            Checkpoint checkpoint = CreateCheckpoint(row, currentDateString, currentHourString, ref currentCell);
            currentCell--;
